...
var collapseRegex = /^JSON|concat|none|extract$/;
...
function readTarget(record, serviceDefinitions) {
  ...
  if(((typeof target.collapse) != "string") || (! collapseRegex.test(target.collapse))) {
    console.error("Invalid configuration for target '" + target.id + "' of type '" + target.type + "', 'collapse' property must be a valid collapse type (JSON, concat, concat-b64, none, extract). Ignoring target");
    return null;
  }
